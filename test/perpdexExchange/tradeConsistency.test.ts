...
describe("PerpdexExchange trade consistency", () => {
    ...
    ;[0, 5e4].forEach(fee => {
        ;[0, 1e4].forEach(protocolFee => {
            ;[false, true].forEach(withLimitOrders => {
                ;[false, true].forEach(isBaseToQuote => {
                    ;[false, true].forEach(isExactInput => {
                        ;[false, true].forEach(isLiquidation => {
                            describe(`fee ${fee} protocolFee ${protocolFee} withLimitOrders ${withLimitOrders} isBaseToQuote ${isBaseToQuote} isExactInput ${isExactInput} isLiquidation ${isLiquidation}`, () => {
                                let amount
